...
export default function TemplatesModal({ isOpen, onClose, yShapes, viewportCenter = { x: 0, y: 0 } }) {
  ...
  const handleApplyTemplate = () => {
    ...
    // We offset the templates by the viewport center so they appear nicely in view.
    const newShapes = templateToConfirm.shapes.map(shape => {
      const uniqueId = Math.random().toString(36).substr(2, 9);
      const isArrow = shape.type === 'arrow';
      
      // If it's an arrow, we need to translate the points array
      if (isArrow && shape.points) {
        return {
          ...shape,
          id: uniqueId,
          points: shape.points.map((p, i) => i % 2 === 0 ? p + viewportCenter.x : p + viewportCenter.y)
        };
      }

      // Default X and Y translation
      return {
        ...shape,
        id: uniqueId,
        x: (shape.x || 0) + viewportCenter.x,
        y: (shape.y || 0) + viewportCenter.y
      };
    });
              ...
              {filteredTemplates.map(template => (
                ...
                >
                  <div className="h-40 bg-gray-200 relative overflow-hidden">
                    {/* Placeholder image, falls back to a nice gradient if not found */}
                    <img 
